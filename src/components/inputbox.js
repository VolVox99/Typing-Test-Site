...
class InputBox extends Component {
    ...
    constructor(props) {
        ...
        this.sounds = [
            [
            new UIfx(key_press_1, {volume: this.state.volume, throttleMs: this.throttleMs}),
            new UIfx(key_press_2, {volume: this.state.volume, throttleMs: this.throttleMs}),
            new UIfx(key_press_3, {volume: this.state.volume, throttleMs: this.throttleMs}),
            new UIfx(key_press_4, {volume: this.state.volume, throttleMs: this.throttleMs}),
            new UIfx(key_press_5, {volume: this.state.volume, throttleMs: this.throttleMs}),
            new UIfx(key_press_6, {volume: this.state.volume, throttleMs: this.throttleMs}),
            ],
        
            [
            new UIfx(space_press, {volume: this.state.volume, throttleMs: this.throttleMs}),
            new UIfx(backspace, {volume: this.state.volume, throttleMs: this.throttleMs}),
            ]
        ]
    ...
    updateText(event) {
        ...
        this.setState(prevState => {
            ...
            window.text_difference = ((text_input ? text_input.length : 0) - (prevState.text ? prevState.text.length : 0))
            return {
            text: text_input,
            started_typing: true,
    ...
    adjustedWPM(seconds){
        ...
        let total_chars = correct_indexes.reduce((prevSum, index) => prevSum + this.prompt_words[index].length, 0)
    ...
    checkCurrentWordWrong() {
        ...
        let prompt_word = current_typed_words ? this.prompt_words[this.state.word_index] : ''
    ...
    accuracy(){
        ...
        if(this.state.started_typing) {
            let acc = Math.floor(100 - (100 * ((this.state.prompt_word_typed_wrong_indexes ?? []).length/(this.state.word_index  || 1))))
            if(acc < 0) return 0
            return acc
    ...
    componentDidMount = () => {
        ...
        setInterval(() => {
            ...
            this.setState(prevState => ({
                ...
                WPM: this.state.seconds_passed ? this.adjustedWPM(this.state.seconds_passed) : 0,
                word_index: this.wordsIndex(),
    ...
    restart(self) {
        ...
        if(self.prompt_box && self.prompt_box.current && self.prompt_box.current.scrollTop){
    ...
    scrollPromptDown() {
        ...
        if(this.state.word_index >= 1){
            let current_y = this.current_word && this.current_word.current.getBoundingClientRect().y
            let prev_y = this.prev_word && this.prev_word.current.getBoundingClientRect().y
            ...
            if(distance > 0 && this.state.word_index > this.last_word_index){
                    ...
                    this.prompt_box.current.scrollTop += (this.first_scrolled? distance: distance * 1.25)
            ...
            else if(this.last_distance > 0 && this.state.word_index < this.last_word_index){
    ...
    render() {
        ...
        if (this.state.done) {
            ...
            return <Redirect to={{
                pathname: '/completed',
                state: { WPM: this.state.WPM, words_wrong: this.state.prompt_word_typed_wrong_indexes.length, accuracy: this.state.accuracy}
            }} />
        }
